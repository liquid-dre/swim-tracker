---
target: Qualification status chart page (R2)
total_score: 37
p0_count: 0
p1_count: 0
timestamp: 2026-07-02T05-55-12Z
slug: components-status-statusmatrixscreen-tsx
---
## Design Health Score — Qualification status (chart page, R2)

Anchor: **the qualification status matrix**. Filters collapsed into one slim toolbar (primary inline: none (the matrix is all data)) with secondary filters (gender + age band + squad) behind a compact Filters popover carrying an active-count badge. The tall filter card that used to push the chart down is gone.
...
1. **The chart is the hero.** Removing the tall filter card and collapsing controls into one slim toolbar puts the qualification status matrix at the top of the page, centred and dominant above the fold.
