---
target: Progression chart page (R2)
total_score: 37
p0_count: 0
p1_count: 0
timestamp: 2026-07-02T05-55-12Z
slug: components-progression-progressionscreen-tsx
---
## Design Health Score — Progression (chart page, R2)

Anchor: **the multi-line progression chart (inverted axis)**. Filters collapsed into one slim toolbar (primary inline: mode toggle + swimmer / group + event, inline) with secondary filters (squad filter + name search, inside the Swimmers popover) behind a compact Filters popover carrying an active-count badge. The tall filter card that used to push the chart down is gone.
...
| **Total** | | **37/40** | **Good — chart-first, ship it** |
...
1. **The chart is the hero.** Removing the tall filter card and collapsing controls into one slim toolbar puts the multi-line progression chart (inverted axis) at the top of the page, centred and dominant above the fold.
