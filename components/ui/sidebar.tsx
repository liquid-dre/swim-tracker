...
function SidebarTrigger({
  ...
  // Plain ghost icon-button (our token vocabulary) — no second Button kit.
  return (
    <button
      type="button"
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        // min-w-0 is load-bearing: as a flex item its min-width would otherwise
        // default to its content's intrinsic size, so a wide child (e.g. the
...
function SidebarInput({
  ...
  return (
    <input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full rounded-md border border-border bg-background px-2 text-sm text-ink outline-none placeholder:text-ink-muted focus-visible:border-border-strong focus-visible:ring-2 focus-visible:ring-ring",
        className
      )}
      {...props}
    />
...
function SidebarMenuSkeleton({
  ...
  // Varied-but-deterministic width (50–85%) so skeleton rows aren't uniform,
  // without an impure Math.random() in render. Derived from a stable id.
  const id = React.useId()
  const width = React.useMemo(() => {
    let hash = 0
    for (let i = 0; i < id.length; i++) hash = (hash * 31 + id.charCodeAt(i)) | 0
    return `${50 + (Math.abs(hash) % 36)}%`
  }, [id])
