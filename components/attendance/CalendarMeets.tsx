...
export function MeetPinChip({
  ...
  return (
    <button
      ...
      aria-label={title}
      className={cn(
        "tap flex w-full items-center gap-1 rounded-md border px-1.5 py-1 text-2xs sm:text-xs outline-none",
        ...
        {current?.kind === "meet" ? (
          <>
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto px-4 py-1">
              ...
              {/* `meet` too, so a coach checking a clash sees the day sections
                  a multi-day gala's programme is split into — which is the
                  whole question they opened the calendar with. */}
              <MeetProgrammeTable
                events={current.meet.events}
                meet={current.meet}
                headingLevel={3}
              />
            </div>
